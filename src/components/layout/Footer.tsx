...
const SOCIAL = [
  ...
  { key: 'tiktok',    label: 'TikTok',    icon: <TtIcon />, cls: 'bg-fg border border-border' },
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-10">
              ...
              <div className="flex gap-2">
                {SOCIAL.map(({ key, label, icon, cls }) => (
                  <Link
                    ...
                    aria-label={label}
                    className={`${cls} w-9 h-9 rounded-xl flex items-center justify-center text-white hover:opacity-80 transition-opacity`}
                  >
                    {icon}
                  </Link>
                ))}
          ...
          <div className="lg:col-span-3 grid grid-cols-2 sm:grid-cols-4 gap-8">
            {NAV_COLS.map(({ heading, links }) => (
              <div key={heading}>
                <h3 className="label-eyebrow text-brand-fg/40 mb-4">{heading}</h3>
                <ul className="flex flex-col gap-2.5">
                  {links.map(({ label, href }) => (
                    <li key={href}>
                      <Link
                        href={href}
                        className="text-sm text-brand-fg/50 hover:text-brand-fg transition-colors"
                      >
                        {label}
                      </Link>
                    </li>
      ...
      <div className="border-t border-border/20">
        <div className="container-max px-4 py-4 flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-4">
            ...
            <Link href="/terms" className="text-xs text-brand-fg/30 hover:text-brand-fg/60 transition-colors">
              Terms
            </Link>
            <span className="text-xs text-brand-fg/20">Rio Grande Valley, TX</span>
          </div>
        </div>
      </div>
          ...
          {[
            ...
            <Link
              ...
              className={`
                ...
                ${primary ? 'bg-brand hover:opacity-90' : 'hover:bg-muted'}
              `}
            >
              <span className="text-lg leading-none">{icon}</span>
              <span className={`text-[10px] font-semibold leading-tight ${primary ? 'text-brand-fg' : 'text-muted-fg'}`}>
                {label}
              </span>
            </Link>
          ))}
