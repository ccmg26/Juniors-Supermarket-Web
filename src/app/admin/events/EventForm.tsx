...
export default function EventForm({ event }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 max-w-xl">
      ...
      <div>
        <label className="block text-sm font-semibold text-brand-black mb-1">Event Title *</label>
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-semibold text-brand-black mb-1">Start Date *</label>
          <input type="date" name="start_date" defaultValue={event?.start_date?.split("T")[0]} required className="input-base" />
        </div>
        <div>
          <label className="block text-sm font-semibold text-brand-black mb-1">End Date *</label>
          <input type="date" name="end_date" defaultValue={event?.end_date?.split("T")[0]} required className="input-base" />
        </div>
      </div>
      <div className="flex gap-6">
        <label className="flex items-center gap-2 cursor-pointer">
          <input type="checkbox" name="is_featured" value="true" defaultChecked={event?.is_featured ?? false} className="rounded border-gray-300 text-brand-red focus:ring-brand-red" />
          <span className="text-sm font-semibold text-brand-black">Featured</span>
        </label>
        <label className="flex items-center gap-2 cursor-pointer">
          <input type="checkbox" name="is_active" value="true" defaultChecked={event?.is_active ?? true} className="rounded border-gray-300 text-brand-red focus:ring-brand-red" />
