  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5 max-w-2xl">
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-semibold text-fg mb-1">Store Name *</label>
          <input name="name" defaultValue={store?.name} required className="input-base" placeholder="Edinburg University" />
        </div>
        <div>
          <label className="block text-sm font-semibold text-fg mb-1">Slug *</label>
          <input name="slug" defaultValue={store?.slug} required className="input-base" placeholder="edinburg-university" />
        </div>
      </div>

      <div>
        <label className="block text-sm font-semibold text-fg mb-1">Street Address *</label>
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-semibold text-fg mb-1">Phone *</label>
          <input name="phone" defaultValue={store?.phone} required className="input-base" placeholder="(956) 000-0000" />
        </div>
        <div>
          <label className="block text-sm font-semibold text-fg mb-1">Hours</label>
          <input name="hours" defaultValue={store?.hours ?? "Open Daily 7:00 AM – 10:00 PM"} className="input-base" />
        </div>
      </div>

      <div>
        <label className="block text-sm font-semibold text-fg mb-1">Google Maps URL</label>
        ...
        <div className="flex flex-wrap gap-2">
          {STORE_SERVICES.map((svc) => (
            <label key={svc} className="flex items-center gap-1.5 cursor-pointer">
              ...
              />
              <span className="text-sm text-fg">{svc}</span>
            </label>
          ))}
        </div>
      </div>
      ...
      <div className="flex items-center gap-6">
        <label className="flex items-center gap-2 cursor-pointer">
          <input
            ...
            className="rounded border-border text-brand focus:ring-brand"
          />
          <span className="text-sm font-semibold text-fg">EBT/WIC Accepted</span>
        </label>
        <label className="flex items-center gap-2 cursor-pointer">
          <input
            type="checkbox"
